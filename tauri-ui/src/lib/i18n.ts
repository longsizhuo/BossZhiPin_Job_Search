...
const zh: Dict = {
  ...
  "config.letterPrompt": "招呼语 Prompt",
  "config.letterPromptHint": "自定义「怎么写招呼语」的指令。留空 = 用内置默认（口语、简短、像真人）。{usr_name} 会替换成你的名字。",
  "config.letterPromptPlaceholder": "留空 = 用内置默认 prompt（下面灰字就是默认内容）",
...
const en: Dict = {
  ...
  "config.language": "界面语言 · Language",
  "config.letterPrompt": "Greeting Prompt",
  "config.letterPromptHint": "Customize the instruction for writing greetings. Empty = built-in default (casual, short, human-like). {usr_name} is replaced with your name.",
  "config.letterPromptPlaceholder": "Empty = use the built-in default prompt (shown greyed out below)",
