...
- [ ] [`models/llm.py`](../../models/llm.py) `PROVIDERS` 加一条
- [ ] [`main.py`](../../main.py) `PROVIDER_ENV_KEYS` + `PROVIDER_SIGNUP` 加一条
- [ ] [`.env.example`](../../.env.example) 加注释 + 申请链接
- [ ] [`audit/telemetry.py`](../../audit/telemetry.py) `PRICING_CNY_PER_M_TOKENS` 加一行
